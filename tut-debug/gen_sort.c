...
enum { recno_len = 10 };
enum { pad_len = 80 };
enum { rec_len = key_len + pad_len + recno_len };	//BUG #8

enum { buf_nrec = 1 << 4 };				
enum { buf_size = buf_nrec * 100 };			//BUG #4
...
main (int argc, char *argv[])
{
	...
	/* prepare buf */
	char *buf = (char *)malloc(buf_size*(1<<16)) ;		//BUG #7
	if (buf  == NULL) {
		printf("error: unable to malloc buffer\n");
		exit(EXIT_FAILURE);
	}

	FILE *file = fopen(fn, "w");			//BUG #5
	if (!file){					//BUG #3
	...
	while (cur_rec < rec_max) {
		long cnt = fill_buf(buf);
		long ret = (long)fwrite(buf, 1, cnt * rec_len, file);	//BUG #10
		if (ret != cnt * rec_len) {
