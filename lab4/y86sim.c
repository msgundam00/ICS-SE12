...
long_t compute_alu(alu_t op, long_t argA, long_t argB)
{
    ...
    switch(op){
		...
		case A_XOR:
			...
			break;
		default:
			val = argA;				//----------------------unreasonable no cc set
	}
    return val;
}
...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
    ...
    bool_t sign = FALSE;
    bool_t ovf = FALSE;
	/*For ZF and SF, all the ALUs have the same pattern*/
	if(val == 0)
		zero = TRUE;
	if(val < 0)
		sign =  TRUE;
		
	/*But OF is a little different*/
	switch(op){
		case A_ADD:
	...
	if (op == A_SUB){
		...
		if(((argA >> 31 ) != (argB >> 31) && ((argB >> 31 ) != (val >> 31)) && (argA != 0 ) && (argB != 0)))
			ovf = TRUE;
		if(val == 0)
			zero = TRUE;
		if(val < 0)
	...
	*/
    return PACK_CC(zero,sign,ovf);				//-----------seems done
}
...
bool_t cond_doit(cc_t cc, cond_t cond)
{
	...
	switch (cond){
		case C_YES:
			...
			break;
		case C_LE:
			condi = ((GET_SF(cc) ^ GET_OF(cc)) | GET_ZF(cc));
			break;
		case C_L:
			condi = (GET_SF(cc) ^ GET_OF(cc));
			break;
		case C_E:
			condi = (GET_ZF(cc));
			break;
 ...
 *     STAT_AOK: continue
 *     STAT_HLT: halt
 *     STAT_INS: invalid instruction address
 *     STAT_ADR: invalid instruction, register id, data address, stack address, ...
 */
stat_t nexti(y86sim_t *sim)
{
	...
	byte_t regAB = 0;
	get_byte_val(sim->m, next_pc, &regAB);
	...
	switch(icode){
		...
		case I_JMP:
		case I_CALL:
			if(get_long_val(sim->m, next_pc, &imm) == TRUE)
                next_pc+=4;
            else
                return STAT_ADR;
			break;
	...
	long_t regAval = get_reg_val(sim->r, regA);
	long_t regBval = get_reg_val(sim->r, regB);
    switch (icode) {
      case I_HALT: /* 0:0 */
        return STAT_HLT;
        break;
      ...
      case I_RRMOVL:  /* 2:x regA:regB ,including condition move*/
		sim->pc = next_pc;
		bool_t conditionRR = cond_doit(sim->cc, ifun) ;
		if(conditionRR == TRUE){
			set_reg_val(sim->r, regB, regAval);
		}
		break;
      ...
      case I_RMMOVL: /* 4:0 regA:regB imm */
		sim->pc = next_pc;
		if(set_long_val(sim->m, regBval + imm,regAval) == FALSE){
		    sim->pc -=6;
			sim->badmem = regBval + imm;
			return STAT_ADR;
        }
		break;
      ...
      case I_MRMOVL: /* 5:0 regB:regA imm */
		sim->pc = next_pc;
		if(get_long_val(sim->m,regBval + imm, &regAval) == FALSE){
			sim->pc -=6;            //roll back pc
			sim->badmem = regBval + imm;
			return STAT_ADR;        //set stat to STAT_ADR
		}
		set_reg_val(sim->r, regA, regAval);
		break;

		case I_ALU: /* 6:x regA:regB */
        sim->pc = next_pc;
        long_t result = compute_alu(ifun,regAval,regBval);
		sim->cc =  compute_cc(ifun, regAval, regBval,result);
		set_reg_val(sim->r, regB, result);
        break;

      case I_JMP: /* 7:x imm */
		sim->pc = next_pc;
		bool_t conditionJMP = cond_doit(sim->cc, ifun) ;
		if(conditionJMP == TRUE){
			sim->pc = imm;
		}
		break;
      ...
      case I_CALL: /* 8:x imm */
	  	sim->pc = next_pc;
	  	long_t callESPval = get_reg_val(sim->r, REG_ESP);	//get %esp
		set_reg_val(sim->r, REG_ESP, callESPval-4);			//subl $0x4,%esp
		if(get_reg_val(sim->r, REG_ESP) < 0){				//Invalid stack address
			sim->isStack = TRUE;
			sim->badmem = get_reg_val(sim->r, REG_ESP);
			sim->pc -= 5;
			return STAT_ADR;
		}
		set_long_val(sim->m, callESPval-4, next_pc);		//move %e??,(%esp)
		sim->pc = imm;										//jmp 0xaddress
		break;

      case I_RET: /* 9:0 */
        sim->pc = next_pc;
        get_long_val(sim->m,get_reg_val(sim->r, REG_ESP),&(sim->pc));		//set %eip to ret. addr.
		set_reg_val(sim->r, REG_ESP, get_reg_val(sim->r, REG_ESP)+4);		//addl $0x4,%esp
		break;
      case I_PUSHL: /* A:0 regA:F */
	  	sim->pc = next_pc;
	  	long_t pushESPval = get_reg_val(sim->r, REG_ESP);	//get %esp
	  	set_reg_val(sim->r, REG_ESP, pushESPval-4);			//subl $0x4,%esp
        if(get_reg_val(sim->r, REG_ESP) < 0){				//Invalid stack address
			sim->isStack = TRUE;
			sim->badmem = get_reg_val(sim->r, REG_ESP);
			sim->pc -= 2;
		...
		set_long_val(sim->m, pushESPval-4, regAval);		//move %e??,(%esp)
		break;
      case I_POPL: /* B:0 regA:F */
	  	sim->pc = next_pc;
		long_t poplDestination = 0;
		long_t poplESPval = get_reg_val(sim->r, REG_ESP);	//get %esp
		get_long_val(sim->m, poplESPval ,&poplDestination);	//get (%esp)
		set_reg_val(sim->r, REG_ESP, poplESPval +4);		//addl $0x4,%esp
		set_reg_val(sim->r, regA, poplDestination);			//move (%esp),%e??
		break;
...
int main(int argc, char *argv[])
{
    ...
    if(e == STAT_ADR){
		if(sim->isStack)
			printf("PC = 0x%x, Invalid stack address 0x%x\n", sim->pc,sim->badmem);
		else
			printf("PC = 0x%x, Invalid data address 0x%x\n", sim->pc,sim->badmem);
	}
