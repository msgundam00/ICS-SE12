...
#    			DESCRIPTION
# Name: Liu Che
# Student ID: 5120379066
#	
#  Description:
#	1.Adding iaddl and ret into instruction set. The same
#	  as seq-full.hcl
#	2.Optimizing instruction "ret" in controlling logic
#	  "ret" appears in function "ncopy" after instruction
#	  "mrmovl" and "pushl" with the same register used, the
#	  return address can be forwarded from decode stage to 
#	  memory stage.
#	3.Implemented load forward to avoid Load/Use hazard.
#	4.Implemented backward taken, forward not taken
#	5.Optimization for PIPE simulator: Combine andl/jg/iaddl to
#	  one instruction. Immediately after rmmovl get the value of 
#	  %edi, this improved "andl" can function as "%esi += abs(%edi)"
#	  BUT in SEQ simulator, this won't work.
#
#	
#	
####################################################################
...
intsig REAX     'REG_EAX'    	     # Stack Pointer
...
intsig D_ifun 'if_id_curr->ifun'   # Instruction code
...
intsig D_valC 'if_id_curr->valc'     # Incremented PC
...
intsig m_valE 'mem_wb_next->vale'	# valM generated by memory
...
#	M_icode == IJXX && !M_Cnd : M_valA;
	...
	W_icode == IRET && E_icode==INOP: W_valM;
...
int f_ifun = [
	imem_error : FNONE;
	imem_icode == IOPL && imem_ifun == 2 && D_icode == IOPL && E_icode == IOPL && D_ifun == E_ifun && D_ifun == 0 : ALUXOR ;
	1: imem_ifun;
];
...
int d_srcB = [
	D_icode == IOPL && D_ifun == 2 && E_icode == IRMMOVL &&M_icode == IMRMOVL : REAX ;
...
int aluA = [
	...
	E_icode == IOPL && E_ifun == 2 && M_icode == IRMMOVL && W_icode == IMRMOVL && E_valA > 0 : 1 ;
	E_icode == IOPL && E_ifun == 2 && M_icode == IRMMOVL && W_icode == IMRMOVL && E_valA <= 0 : 0 ;
...
int aluB = [
	E_icode in { IRMMOVL, IMRMOVL, IOPL, ICALL, 
		     IPUSHL, IRET, IPOPL , IIADDL, ILEAVE, IJXX } : E_valB;
...
int alufun = [
	E_icode == IOPL && E_ifun == 2 && M_icode == IRMMOVL && W_icode == IMRMOVL : ALUADD ;
	E_icode == IOPL : E_ifun;
	1 : ALUADD;
];
...
int M_valA = [
	M_icode == IMRMOVL && E_icode in {IRMMOVL} &&
	 	 M_dstM in {E_srcA} : m_valM;
...
bool D_bubble =
	...
	!(E_icode in { IMRMOVL, IPOPL ,ILEAVE } && 
	E_dstM in { d_srcA, d_srcB })&&
	!(E_icode in {IMRMOVL} && D_icode in {IRMMOVL} &&
		 E_dstM in {d_srcA} ) &&
	  	(IRET == D_icode) ||
	(IRET == E_icode && IPUSHL!=M_icode) ||
	(IRET == M_icode && IPUSHL!=W_icode) ;
